...
public interface AccommodationRepository extends JpaRepository<Accommodation, String>, JpaSpecificationExecutor<Accommodation> {

    public static final String FIND_BY_LAT_LONG = """
            SELECT * FROM accommodations
            WHERE lat = ?1 AND lon = ?2 AND is_active = true
            """;

    public static final String FIND_BY_ACCOMMODATION_NAME = """
            SELECT * FROM accommodations
            WHERE accommodation_name %?1% AND is_active = true
            """;

    public static final String FIND_BY_USER_ID = """ 
            SELECT * FROM accommodations
            WHERE user_id = ?1 AND is_active = true
            JOIN accommodation_images ON accommodations.id = accommodation_images.accommodation_id
            """;
    ...
    public static final String FIND_BY_ACC_ID = """
            ...
            WHERE a.id = :id AND is_active = true
            """;
