...
public class ReservationAdaptor {
    ...
    private boolean checkNewCheckInCheckOutCollisionWithOtherUser(Date checkIn, Date checkOut) {
        reservationService.getReservationByCheckInCheckOut(checkIn, checkOut);
        return reservationService.getReservationByCheckInCheckOut(checkIn, checkOut).isEmpty();
    }
    ...
    public ReservationResponse updateReservation(String reserve_id, ReservationRequest r) throws Exception {
        ...
        Date new_checkIn = r.getCheckIn();
        Date new_checkOut = r.getCheckOut();

        // check new check in and out are collision with other user
        if (checkNewCheckInCheckOutCollisionWithOtherUser(new_checkIn, new_checkOut)) {
            throw ReservationException.repeatBooking();
        }
        ...
        if (!user.getId().equals(reservation.getUser().getId())) {
            throw ReservationException.notOwner();
        }
        ...
        reservation.setTotal_nights(reserveDays);
